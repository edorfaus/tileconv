...
func TestTileRowPairPlanarEncode(t *testing.T) {
	...
	check := func(t *testing.T, bd tileconv.BitDepth, x, y int) {
		want := td.TileRowPairPlanar(x, y, bd.Planes())
		src := td.FullImage()

		c := tileconv.TileRowPairPlanar{BitDepth: bd}
...
func TestTileRowPairPlanarDecode(t *testing.T) {
	...
	var failCount, testDataPlanes int
	const maxBadPix = 2
	const maxFails = 2
	...
	check := func(t *testing.T, bd tileconv.BitDepth, x, y int) {
		...
		src := td.TileRowPairPlanar(x, y, testDataPlanes)
		got := td.BaseImage()

		tp := tileconv.TileRowPairPlanar{BitDepth: bd}
		tp.Decode(src, got, x, y)

		goodSrc := td.TileRowPairPlanar(x, y, testDataPlanes)
	...
	for i, rng := range []bool{false, true} {
		t.Run(names[i], func(t *testing.T) {
			...
			for bd := tileconv.BD1; bd <= tileconv.BD8; bd++ {
				testDataPlanes = bd.Planes()
				runTestForDepth(t, bd)
			}
		})
	}

	// Test odd bit depths with non-zero last plane (aka bad data)
	t.Run("BadOdd", func(t *testing.T) {
		td = newTestImageData(true)
		fullImage = td.FullImage()
		baseImage = td.BaseImage()

		for bd := tileconv.BD1; bd <= tileconv.BD8; bd += 2 {
			testDataPlanes = bd.Planes() + 1
			runTestForDepth(t, bd)
		}
	})
}
